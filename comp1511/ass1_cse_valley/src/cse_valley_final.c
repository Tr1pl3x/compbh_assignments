...
void disaster(char drought, struct land farmland[LAND_SIZE][LAND_SIZE], int num);
int drought_test(char drought,struct land farm_land[LAND_SIZE][LAND_SIZE], int kill_col[64], int kill_row[64], int num);
int surround_plant(int i, int j, struct land farm_land[LAND_SIZE][LAND_SIZE]);
...
int main(void) {
    ...
    while ( cmd != EOF ) {
        ...
        if (cmd == 'a') {
            ...
            scanf(" %c", &drought);
            int surround_num;
            scanf(" %d", &surround_num);
            disaster( drought,farm_land, surround_num);
...
void disaster(char drought, struct land farm_land[LAND_SIZE][LAND_SIZE], int num) {


    int kill_row[64] = {-1};
    int kill_col[64] = {-1};
    int size = drought_test(drought, farm_land,kill_col, kill_row, num);
    
    for( int i = 0; i < size; i++) {

        farm_land[kill_row[i]][kill_col[i]].seed_name = NO_SEED;
    }
    ...
    for ( int i = 0; i < LAND_SIZE; i++) {
        ...
        for ( int j = 0; j < LAND_SIZE; j++) {
            ...
            if( farm_land[i][j].seed_name != NO_SEED ) {
                ...
                int count = surround_plant(i , j, farm_land);
                if ( drought == 'd') {
...
int surround_plant(int i, int j, struct land farm_land[LAND_SIZE][LAND_SIZE]) {
    ...
    start_row = boundary_test(start_row);
    start_col = boundary_test(start_col);
    end_row = boundary_test(end_row);
    end_col = boundary_test(end_col);
    ...
    while ( r <= end_row) {
        ...
        while ( c <= end_col) {
            ...
            if ( farm_land[r][c].seed_name != NO_SEED) {
